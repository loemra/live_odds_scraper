...
def get_events_urls(date: datetime) -> list[str]:
    ...
    for sport in _config["sports"]:
        event_urls.append(_config["get_events_url"].format(sport, date.strftime(r"%Y-%m-%d")))
    return event_urls


def get_event_url(id: str, sport: str) -> str:
    markets = ",".join([urllib.parse.quote_plus(market) for market in _config["sports"][sport]["markets"]])
