...
def _maybe_match_event(
    ...
    relevant_events = [
        ...
        if abs((sportsbook_event.date - event.date).total_seconds()) < 3600
    ]
    ...
    _logger.debug(
        f"{sportsbook_event} has multiple best matches {best_matches}"
    )
...
def _unify_event(sportsbook: str, event: EventMetadata) -> EventMetadata:
    unified_name = translater.sportsbook_to_unified_event_name(
        sportsbook, event.name
    )
    if not unified_name:
    ...
    return EventMetadata(
        _make_unified_id(), event.name, event.sport, event.date
    )
...
def _maybe_match_selection(
    sportsbook: str,
    sportsbook_selection: SelectionMetadata,
    unified_selctions: list[SelectionMetadata],
    ...
    if len(best_matches) == 1:
        _logger.debug(
            f"successful match: {sportsbook_selection}, {best_matches}"
        )
        return best_matches[0][0]

    _logger.debug(
        f"{sportsbook_selection} has multiple best matches {best_matches}"
    )

    return _prompt_for_match(sportsbook_selection, unified_selctions)


def _unify_selection(
    sportsbook: str, selection: SelectionMetadata
) -> SelectionMetadata:
    unified_name = translater.sportsbook_to_unified_selection_name(
        sportsbook, selection.name
    )
    if not unified_name:
...
def _update_selection(
    ...
    unified_selection_id = translater.sportsbook_to_unified_selection_id(
        sportsbook, selection.metadata.id
    )
    if not unified_selection_id:
        ...
        translater.maybe_register_selection_id(
            sportsbook, selection.metadata.id, unified_selection.id
        )
        translater.maybe_register_selection_name(
            sportsbook, selection.metadata.name, unified_selection.name
        )
    ...
    events_database.update_event_odds(
        Update(
            ...
            sportsbook,
            selection.odds[sportsbook],
        )
    )


def _update_market(sportsbook: str, unified_event_id: str, market: Market):
    unified_market_id = translater.sportsbook_to_unified_market_id(
        sportsbook, market.metadata.id
    )
    if not unified_market_id:
        _logger.warning(
            f"unable to translate market {market.metadata} for {sportsbook}"
        )
        return
    ...
    for selection in market.selection.values():
        _update_selection(
            sportsbook, unified_event_id, unified_market_id, selection
        )
...
def _update_event(
    ...
    unified_event_id = translater.sportsbook_to_unified_event_id(
        sportsbook, event.id
    )
    if not unified_event_id:
        unified_sport = translater.sportsbook_to_unified_sport(
            sportsbook, event.sport
        )
        if not unified_sport:
            _logger.error(
                f"unable to find unified sport {event.sport} for {sportsbook}"
            )
            return
        ...
        translater.maybe_register_event_id(
            sportsbook, event.id, unified_event_id
        )
        translater.maybe_register_event_name(
            sportsbook, event.name, unified_event.name
        )
