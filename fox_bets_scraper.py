...
def _get_live_events():
    ...
    if result.status_code != 200:
        logger.error(f'get_live_events status code: {result.status_code}\nresult: {result.text}')
        exit(-1)
...
def _get_live_markets_for_sports(sports):
    markets = []
    for sport in sports:
        markets.append(','.join(config['markets_for_sports'][sport]['keyMarkets']['inplay']))
    return markets

def generate_payloads():
    live_events = _get_live_events()
    event_ids, sports = map(list, zip(*_get_event_id_and_sport_from_live_events(live_events)))
